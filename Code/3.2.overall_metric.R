...
#################################################################################
# Must match ENS_K in Code/2.5.TabPFN-B.py (TabPFN-B1.txt … TabPFN-B{ENS_K}.txt)
#################################################################################
ENS_K <- 10L
...
# Same combo math as Code/3.3.MW-Stats.R (combo_cell_latex)
...
# Ensemble RMSE table: C1 … C_K then TabPFNB last (mean of C1 … C_K; Code/2.5.TabPFN-E.py).
ensemble_col_keys <- c(
  stats::setNames(paste0("x_C", seq_len(ENS_K)), paste0("C", seq_len(ENS_K))),
  stats::setNames("x_E", "TabPFN-B")
)
...
#################################################################################
# Wide frame: join TabPFN-B1 … TabPFN-B_K on reference grid (first method_outputs_main file).
#################################################################################
build_ensemble_wide <- function() {
  ...
  if (any(!ccols %in% names(out))) {
    stop("Missing TabPFN-B prediction columns after join; check TabPFN-B*.txt")
  }
...
cap_rmse_ens <- paste0(
  ...
  "} are bootstrap ensemble members (\\texttt{TabPFN-B\\textit{m}.txt}, Code/2.5.TabPFN-B.py); ",
  "\\textbf{TabPFN-B} (last column) is their row-wise mean. ",
...
lines_doc <- c(
  "% Generated by Code/3.2.overall_metric.R — do not edit by hand.",
  "% Tables: (1) MBE and RMSE on one row per combo (Raw + MLR–TabPFN; MBE columns | RMSE columns)",
  "%         (2) RMSE C1–C_K + TabPFN-B (last).",
  "% \\input{tex/baseline_tables} from your main .tex (adjust path).",
  "% Requires TabPFN-B1.txt … TabPFN-B{K}.txt in Data/Output (K = ENS_K in this file).",
